 // /lib/bot-status.ts
    ...
    export async function getBotRunningStatus(): Promise<boolean> {
      try {
        ...
        console.log(`[BotStatusLib] Aktueller Status aus Redis (${BOT_STATUS_KEY}): '${status}' (Typ: ${typeof status})`); // Debug-Log, mit Anführungszeichen und Typ
    ...
    export async function setBotRunningStatus(running: boolean): Promise<void> {
      try {
        const valueToSet = running ? 'true' : 'false'; // Es wird explizit der String 'true' oder 'false' gesetzt
        await redis.set(BOT_STATUS_KEY, valueToSet);
        console.log(`[BotStatusLib] Bot-Status in Redis gesetzt auf: '${valueToSet}'`); // Debug-Log, mit Anführungszeichen
      } catch (error) {
