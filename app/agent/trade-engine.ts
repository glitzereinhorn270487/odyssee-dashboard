 ...
 */
async function findLpAddressForToken(helius: Helius, tokenAddress: string): Promise<string | null> {
  try {
 ...
 */
async function getTokenHolders(helius: Helius, tokenAddress: string): Promise<{ balance: number; owner: string }[]> {
  try {
 ...
 * @returns Das vollständige ScoreXResult-Objekt oder null bei einem Fehler.
 */
export async function decideTrade(token: { address: string; symbol: string; name: string }): Promise<ScoreXResult | null> {
  ...
  try {
    ...
    // KORREKTUR: telegramToggles ist jetzt im Scope, da es vor dem try-Block deklariert wurde
